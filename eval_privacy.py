...
def get_arguments():
    ...
    parser.add_argument('--dataset', default='cifar100',
                        choices=["cifar10", "cifar100", "cifar10_resnet", "cifar100_resnet", "svhn", "locations",
                                 "purchase"])
    parser.add_argument('--save_results', action='store_true', default=False)
    parser.add_argument('--s_model', default=88, type=int, help='s_model')
    parser.add_argument('--t_model', default=89, type=int, help='t_model')
...
if __name__ == "__main__":
    ...
    for aug_i in range(len(aug_types)):
        ...
        for trial in range(args.s_model, args.t_model):
            ...
            print(trial)
            if args.target:
                target = set(samplelist_target)
                y[samplelist_target] = 1
            else:
            ...
            if args.target:
                base = np.load("%s/%s_%s.npy" % (dirs, foldername, "target"))
            else:
                base = np.load("%s/%s_%s.npy" % (dirs, foldername, str(trial)))
            baseeval = []
            for i in range(sample_num):
                baseeval.append(
                    normal(in_dict[i][0], in_dict[i][1], base[i]) - normal(out_dict[i][0], out_dict[i][1], base[i]))

            tpr_0_1, tpr_00_1, tpr_000_1, auroc = ROC_curve(y, baseeval, show=False)
            baseeval = np.array(baseeval)

            # Save baseeval to an npy file
            if args.target:
                np.save(f"sampleinfo/128model_{aug_types}_target_{foldername}.npy", baseeval)
            else:
                np.save(f"sampleinfo/128model_{aug_types}_{str(trial)}_{foldername}.npy", baseeval)
            ...
            AccList = 1 - np.logical_xor(scores[:, np.newaxis] > thr[np.newaxis, :], y[:, np.newaxis]).sum(0) / len(
                scores)
            Acc_opt = np.max(AccList)

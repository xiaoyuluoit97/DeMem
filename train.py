...
def main(cfg, aug_type = "none", index = 0, aug_index = 0):
    ...
    if aug_type in ['distillation', 'smooth', 'mixup']:
        criterion = lambda pred, target: f
    else:
    ...
    trainloader, testloader = get_loaders(args.dataset, aug_type, aug_index, cfg, "resnet18",
    ...
    if args.train:
        ...
        for epoch in range(starting_epoch, cfg['training_num_epochs']):
            ...
            if epoch > 10 and vc_acc < 10.0:
                print('Validation accuracy below 10.0 after epoch 10. Stopping training.')
                break
...
if __name__ == "__main__":
    ...
    results = []
    #noconverage = np.load("non_converage.npy")
    #print("not converage model is")
    #print(noconverage)

    #for i in noconverage:
        #print("current modelis")
        #print(i)
    ...
    for i in need_to_retrain:
        ...
        print("UPUPUPUPUP")
        if not args.train:
            if vc_acc < 40.00 or vc_acc > 60.00:
                results.append(i)
        # except:
        #     print("i:", i)
        #     results.append(i)
    ...
    np.save(f"{args.aug_type}_notconverage.npy", results)
